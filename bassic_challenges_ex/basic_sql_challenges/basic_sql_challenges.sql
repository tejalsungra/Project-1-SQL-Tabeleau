...
SELECT *
FROM  information_schema.coloumns
...
au_fname,
au_lname,
 CONCAT ( au_fname, ' ', au_lname) full_name
FROM publications.authors;
...
-- HINT: parenthesis "()" can help 
 ...
 FROM publications.authors 
   WHERE  
     au_fname = 'Dean'  AND (city = 'Oakland' OR city = 'Berkeley');
...
#LIMIT 3;
SELECT 
stor_id, SUM(qty) FROM publications.sales
GROUP BY stor_id
ORDER BY qty DESC
LIMIT 3;
