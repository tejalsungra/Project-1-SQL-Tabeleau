...
-- An alias lasts only for the set of queries it is called in.
SELECT 
     *,qty 'Quantity'
FROM
    sales;
...
-- DISTINCT only works on one variable at a time. If you use multiple columns, then the combination of those columns will 
-- will be considered as 1 value.
SELECT DISTINCT
(st.city)
FROM
    titles AS t
...
-- This is where the schema is useful to see the relationship between the tables and columns
SELECT 
...
-- Skipping publisher gives the same result because it is an indirect link,
-- but it does not answer the question which specifically asks for indirect links as well.
...
-- exception handling: what to print if a null is encountered
SELECT *,
	CASE
		WHEN qty >= 50 THEN "high sales"
        WHEN 20 <= qty AND qty <50 THEN "medium sales"
        WHEN qty < 20 THEN "low sales" 
...
-- book types: business, trad_cook and psychology
